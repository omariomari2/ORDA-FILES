...
const swipeRoot = document.querySelector('.swipe');
if (swipeRoot) {
  const sections = gsap.utils.toArray('.slide', swipeRoot);
  const images = gsap.utils.toArray('.image', swipeRoot).reverse();
  const slideImages = gsap.utils.toArray('.slide__img', swipeRoot);
  const outerWrappers = gsap.utils.toArray('.slide__outer', swipeRoot);
  const innerWrappers = gsap.utils.toArray('.slide__inner', swipeRoot);
  const count = swipeRoot.querySelector('.count');
  ...
  gsap.set(swipeRoot.querySelector('.slide:nth-of-type(1) .slide__outer'), { xPercent: 0 });
  gsap.set(swipeRoot.querySelector('.slide:nth-of-type(1) .slide__inner'), { xPercent: 0 });
  ...
  swipeRoot.addEventListener('keydown', (e) => {
